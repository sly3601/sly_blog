...
function renderContributionSvg({ username, days, total }) {
  const cell = 10;
  const gap = 3;
  const step = cell + gap;
  const left = 30;
  ...
  const height = 124;
  const defs = contributionWoodSvgDefs();
  ...
  const rects = days.map((day) => {
    ...
    const level = clampNumber(day.level, 0, 4, 0);
    const title = Number.isFinite(day.count) ? `${day.date}: ${day.count} contributions` : `${day.date}: level ${day.level}`;
    return `<rect x="${x}" y="${y}" width="${cell}" height="${cell}" rx="1.7" class="wood-inlay-cell wood-inlay-cell-${level}" fill="url(#woodGrain${level})" stroke="#f0bf70" stroke-opacity="0.52" stroke-width="0.5" filter="url(#inlayShadow)"><title>${escapeXml(title)}</title></rect>`;
  }).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeXml(username)} GitHub contribution calendar">
  <style>
    ...
    .month, .label { fill: #caa576; font-size: 10px; }
    .summary { fill: #f0c88d; font-size: 11px; font-weight: 700; }
  </style>
  ${defs}
  <rect width="100%" height="100%" rx="10" fill="#1b0f08"/>
  <rect width="100%" height="100%" rx="10" fill="url(#contributionBoardGrain)" opacity="0.62"/>
  <circle cx="${width - 26}" cy="14" r="54" fill="url(#contributionCornerGlow)" filter="url(#contributionGlowBlur)" opacity="0.92"/>
  <circle cx="${width - 18}" cy="9" r="18" fill="#fff0bf" opacity="0.22"/>
  ${months}
  ${rows}
  ${rects}
...
function contributionWoodSvgDefs() {
  return `<defs>
    ...
    <pattern id="contributionBoardGrain" patternUnits="userSpaceOnUse" width="42" height="18">
      <path d="M0 3 C11 0,22 7,42 2 M0 12 C10 17,24 8,42 14" fill="none" stroke="#e5aa61" stroke-opacity="0.12" stroke-width="1"/>
      <path d="M0 8 C13 5,27 11,42 7" fill="none" stroke="#120704" stroke-opacity="0.22" stroke-width="0.8"/>
    </pattern>
    ...
    <radialGradient id="contributionCornerGlow" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#fff3bd" stop-opacity="0.9"/>
      <stop offset="26%" stop-color="#ffd27a" stop-opacity="0.46"/>
      <stop offset="62%" stop-color="#c36a1e" stop-opacity="0.16"/>
      <stop offset="100%" stop-color="#c36a1e" stop-opacity="0"/>
    </radialGradient>
    <filter id="contributionGlowBlur" x="-60%" y="-60%" width="220%" height="220%">
